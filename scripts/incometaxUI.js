...
function displayChart(investAmt,gainsAmt){
    const chartData = {
        ...
        data: [investAmt, gainsAmt,gainsAmt+10], 
    };
    ...
    thischart = new Chart(myChart, {
        ...
        const populateUl = () => {
          ...
          TaxableIncome.textContent=`Taxable Income   Rs.${commafy(Math.trunc(total))}`;
          TaxDeductions.textContent=`Tax Deductions   Rs.${commafy(investAmt)}`;
          TaxPayable.textContent=`Tax Payable         Rs.${commafy(Math.trunc(gainsAmt))}`;
        };
